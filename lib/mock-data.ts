    ...
    apps: [
      {
        ...
        processSteps: [
          "Email intake — identify insured name, requestor type (AM, insured, third-party holder)",
          ...
          "If WC with NYSIF: pull certificate from ww3.nysif.com or request via carrier email",
          "If DBL requested: pull DB120.1 from LIDAC or request via carrier email",
          ...
          "QA Review: correct description? endorsements attached? limits match? proper ACORD form?",
    ...
    apps: [
      {
        ...
        processSteps: [
          "Receive monthly renewal marketing sheets via email",
          "Update Renewal Tracker in SharePoint",
          "Account lookup by lookup code in EPIC",
          "Retrieve policy lines and expiration dates (±45 day window)",
          "Determine master account: PROP > XWIN > CWIN > BWIN > WRAP > PKG priority",
          "Create Master Marketing Submission in EPIC (Policies → Marketed → Add)",
          "Add activity: '[Date] Master Marketing Submission' → assign to AM",
          "For Renewal Summaries (120 days): verify billing type (CFIN = Agency Bill)",
          "Pull documents from Attachments (18-month filter): policy copy, endorsements, invoices",
          "Initiate renewal: Actions → Renew → update year, set policy# to TBD, status to REN",
          "Complete RPOL activity → validate Additional Interests vs certificate holders",
        ],
        extractedFields: [
